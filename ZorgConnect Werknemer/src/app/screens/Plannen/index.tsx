...
import { usePlannen } from "./hooks/usePlannen";

export default function Plannen() {
  const { view, setView, selectedShift, openShiftDetail, closeShiftDetail, schedule, staff } =
    usePlannen();
    ...
    <div className="min-h-screen bg-white pb-20 max-w-[390px] mx-auto relative">
      ...
      {view === "Ik" ? (
        <>
          ...
          <div
            onClick={() => openShiftDetail(schedule[0].id)}
        ...
        <>
          ...
          {staff.map((member) => (
            <div
              key={member.id}
              onClick={() => openShiftDetail(member.id)}
              className="px-4 py-4 border-b border-gray-100 cursor-pointer hover:bg-gray-50 active:bg-gray-100"
            >
